...
public class LocalConfigStorage {
    ...
    public static void importConfig() {
        try {
            File cfgFiles = new File(System.getProperty("user.home") + File.separator + "ehackslitecfg.zip");
            ZipInputStream in = new ZipInputStream(new FileInputStream(cfgFiles));
            ZipEntry entry = null;
            byte[] buffer = new byte[2048];
            while ((entry = in.getNextEntry()) != null) {
                FileOutputStream output = null;
                output = new FileOutputStream(new File(Wrapper.INSTANCE.mc().mcDataDir, "/config/ehackslite/" + entry.getName()));
                int len = 0;
    ...
    public static void exportConfig() {
        try {
            ...
            for (IConfiguration config : ConfigurationManager.instance().configurations) {
                ...
                out.putNextEntry(e);
                FileInputStream fis = new FileInputStream(new File(Wrapper.INSTANCE.mc().mcDataDir, "/config/ehackslite/" + config.getConfigFilePath()));
